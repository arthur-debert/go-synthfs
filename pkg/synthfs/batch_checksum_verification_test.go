...
func TestBatchChecksumVerification(t *testing.T) {
	...
	t.Run("Archive operation fails when source files modified", func(t *testing.T) {
		...
		_, err := batch.CreateArchive("backup.tar.gz", synthfs.ArchiveFormatTarGz, "file1.txt", "file2.txt", "file3.txt")
		if err != nil {
	...
	t.Run("Multiple operations with mixed checksum verification results", func(t *testing.T) {
		...
		err = testFS.WriteFile("bad.txt", []byte("Bad file"), 0644)
		if err != nil {
		...
		_, err = batch.Copy("good.txt", "good_copy.txt")
		if err != nil {
			t.Fatalf("Good copy operation failed: %v", err)
		}

		_, err = batch.Copy("bad.txt", "bad_copy.txt")
		if err != nil {
			t.Fatalf("Bad copy operation failed: %v", err)
		}

		// Modify one source file before execution
		err = testFS.WriteFile("bad.txt", []byte("Modified bad file"), 0644)
		if err != nil {
			t.Fatalf("Failed to modify bad file: %v", err)
		}
		...
		// Batch should not be successful due to one failed operation
		if result.Success {
			t.Error("Expected batch to fail due to checksum verification failure")
		}

		// Check individual operation results
		goodOpSucceeded := false
		badOpFailed := false

		for _, opResult := range result.Operations {
			desc := opResult.Operation.Describe()
			if strings.Contains(desc.Path, "good.txt") && opResult.Error == nil {
				goodOpSucceeded = true
			}
			if strings.Contains(desc.Path, "bad.txt") && opResult.Error != nil {
				badOpFailed = true
				if !strings.Contains(opResult.Error.Error(), "checksum verification") {
					t.Errorf("Expected checksum verification error for bad operation, got: %v", opResult.Error)
				}
			}
		}

		if !goodOpSucceeded {
			t.Error("Expected good copy operation to succeed")
		}
		if !badOpFailed {
			t.Error("Expected bad copy operation to fail checksum verification")
		}
	})
	...
	t.Run("Checksum verification with directories is skipped", func(t *testing.T) {
		...
		// Create copy operation with directory (no checksum should be computed)
		op, err := batch.Copy("sourcedir", "destdir")
		if err != nil {
			t.Fatalf("Copy directory operation failed: %v", err)
		}

		// Verify no checksum was computed for directory
		checksum := op.GetChecksum("sourcedir")
		if checksum != nil {
			t.Error("Expected no checksum for directory, but got one")
		}

		// Execute should succeed (no checksums to verify)
		result, err := batch.Run()
		if err != nil {
			t.Fatalf("Batch execution failed: %v", err)
		}

		// Check if operation succeeded or failed for other reasons (implementation-dependent)
		// The important thing is that there should be no checksum verification errors
		for _, opResult := range result.Operations {
			if opResult.Error != nil && strings.Contains(opResult.Error.Error(), "checksum verification") {
				t.Errorf("Unexpected checksum verification error for directory operation: %v", opResult.Error)
			}
		}
	})
}