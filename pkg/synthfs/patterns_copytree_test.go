...
func TestCopyTreePatterns(t *testing.T) {
	...
	t.Run("CopyTreeOperation validation", func(t *testing.T) {
		...
		op := NewCopyTreeOperation("nonexistent", "dest")
		...
		op = NewCopyTreeOperation("file.txt", "dest")
		err = op.Validate(ctx, fs)
		if err == nil {
		...
		op = NewCopyTreeOperation("source", "existing")
		err = op.Validate(ctx, fs)
		if err == nil {
		...
		// Test with overwrite
		op = NewCopyTreeOperation("source", "existing", CopyTreeOptions{Overwrite: true})
		err = op.Validate(ctx, fs)
		if err != nil {
